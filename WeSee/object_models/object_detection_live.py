...
##########################################################################################
# 3. MODEL INFERENCE
# Chỉ sử dụng CPU vì CUDA không có sẵn
providers = ['CPUExecutionProvider']

print("\n=== LOADING MODELS ===")
try:
    # Đường dẫn tới model trong folder models/
    model_path_coco = os.path.join(os.path.dirname(os.path.dirname(__file__)), "models", "yolov10s.onnx")
    print(f"Loading COCO model from: {model_path_coco}")
    session_coco = ort.InferenceSession(model_path_coco, providers=providers)
...
try:
    # Đường dẫn tới custom model trong folder models/
    model_path_custom = os.path.join(os.path.dirname(os.path.dirname(__file__)), "models", "best.onnx")
    print(f"Loading Custom model from: {model_path_custom}")
    session_custom = ort.InferenceSession(model_path_custom, providers=providers)
...
##########################################################################################
# 6. KHỞI TẠO CAMERA
def init_camera(use_cam=None, video_path=None, camera_id=0):
    """Khởi tạo camera - hỗ trợ webcam và video file"""
    # Sử dụng tham số truyền vào hoặc global config
    use_camera = use_cam if use_cam is not None else USE_CAM
    video_file = video_path if video_path is not None else VIDEO_PATH
    
    if use_camera:
        print(f"Trying to initialize camera {camera_id}...")
        try:
...
def start_detection(use_cam=None, video_path=None, camera_id=0):
    """Hàm chính để bắt đầu detection - có thể gọi từ Flask"""
    try:
        # Khởi tạo camera/video
        cap = init_camera(use_cam, video_path, camera_id)
        orig_w = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
        orig_h = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
        
        print(f"Video size: {orig_w}x{orig_h}\n")
        
        # Khởi tạo detection variables
        ...
        while True:
            ...
            # Run custom detection (every 2 frames to optimize performance)
            if frame_count % 2 == 0:
            ...
            for x1, y1, x2, y2, conf, cls in coco_out:
                ...
                # Blur person's face for privacy
                if label == "person":
            ...
            if danger_objects:
                # Sort by distance and take top 5 closest objects
                ...
                # Clear queue if no danger objects
                try:
                    audio_queue.queue.clear()
                except:
                    pass
...
def start_detection_with_camera(camera_id=0):
    """Wrapper function để bắt đầu detection với camera - gọi từ Flask"""
    return start_detection(use_cam=True, camera_id=camera_id)


def start_detection_with_video(video_path):
    """Wrapper function để bắt đầu detection với video file - gọi từ Flask"""
    return start_detection(use_cam=False, video_path=video_path)


# Chạy trực tiếp nếu file được execute
if __name__ == "__main__":
    print("=== DIPR Object Detection Live ===")
    print("Starting detection with default settings...")
    start_detection()
